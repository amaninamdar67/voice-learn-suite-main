-- System Configuration Table (Fixed Version)
...
DROP POLICY IF EXISTS "Admins can view backups" ON system_backups;
DROP POLICY IF EXISTS "Admins can create backups" ON system_backups;
...
-- Create policies for system_config (only admins)
...
-- Create policies for system_backups (only admins)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_system_config_updated_at'
  ) THEN
...
-- Verify the data
SELECT 'System configuration setup complete!' as status;
