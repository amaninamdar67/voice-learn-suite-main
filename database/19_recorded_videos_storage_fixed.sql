-- Create storage bucket for recorded videos (FIXED VERSION)
...
DROP POLICY IF EXISTS "Teachers can upload recorded videos" ON storage.objects;
DROP POLICY IF EXISTS "Anyone can view recorded videos" ON storage.objects;
DROP POLICY IF EXISTS "Teachers can delete their own recorded videos" ON storage.objects;
DROP POLICY IF EXISTS "Teachers can update their own recorded videos" ON storage.objects;
